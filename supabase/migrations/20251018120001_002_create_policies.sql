...
-- NOTE: original policies enforced auth.uid() = author_id. Since `author_id`
-- is being removed from the schema, the enforcement cannot be applied. The
-- following UPDATE policy is permissive and should be reviewed and tightened
-- for production (for example: use a separate ownership mechanism or admin-only updates).
CREATE POLICY "Users can update stories (placeholder)" ON public.stories
